...
function createMetadata(task, ctx) {
  ...
  if (task.tags && task.tags.length > 0) {
    ...
    task.tags.slice(0, MAX_VISIBLE_TAGS).forEach((tag) => {
      const isActive =
        ctx.activeFilters.tag &&
        tag.toLowerCase() === ctx.activeFilters.tag.toLowerCase();
...
function getEmptyMessage(groupKey, hasFilters) {
  if (hasFilters) {
    return {
      ...
      hint: "Try removing one or more filters above.",
    };
  }
  return {
    title: `No tasks for ${GROUP_LABELS[groupKey] || groupKey}.`,
    hint: null,
  };
}
...
export function renderSections(groups, ctx) {
  ...
  sections.forEach((section, index) => {
    ...
    if (groups[groupKey].length === 0) {
      listEl.appendChild(
        createEmptyState(getEmptyMessage(groupKey, ctx.hasActiveFilters)),
      );
    } else {
...
export function renderFilterSummary(ctx) {
  ...
  if (ctx.activeFilters.status !== "all") {
    const statusLabel =
      ctx.activeFilters.status === "active" ? "Active" : "Completed";
  ...
  if (ctx.activeFilters.tag) {
    const chip = createFilterChip(ctx.activeFilters.tag, "#", {
      removable: true,
    });
    chip
      .querySelector(".filter-chip__remove")
  ...
  if (ctx.activeFilters.search) {
    const chip = createFilterChip(ctx.activeFilters.search, "\u223C", {
      removable: true,
    });
    chip
      .querySelector(".filter-chip__remove")
...
export function renderDetailPanel(ctx) {
  ...
  if (task.tags) {
    task.tags.forEach((tag) => {
      ...
      chip
        .querySelector(".tag-chip__remove")
  ...
  if (task.subtasks && task.subtasks.length > 0) {
    body.appendChild(
      createDetailField("Subtasks", task.subtasks.length + " subtasks"),
    );
  }
