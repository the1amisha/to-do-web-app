...
import { createTaskElement, renderSections, renderFilterSummary, renderDetailPanel } from '../js/render.js';
...
describe('createTaskElement', () => {
  ...
  it('adds task--selected when task id matches selectedTaskId', () => {
    const el = createTaskElement(makeTask({ id: 'x' }), makeCtx({ selectedTaskId: 'x' }));
    expect(el.classList.contains('task--selected')).toBe(true);
  });

  it('does not add task--selected when id does not match', () => {
    const el = createTaskElement(makeTask({ id: 'x' }), makeCtx({ selectedTaskId: 'y' }));
  ...
  it('shows no metadata section when task has no metadata', () => {
    const el = createTaskElement(makeTask({ listId: null, dueDate: '', tags: [], subtasks: [] }), ctx);
...
describe('renderSections', () => {
  ...
  it('renders tasks into the correct group sections', () => {
    ...
    const todayList = document.querySelector('[data-group="today"] .content__task-list');
  ...
  it('shows all sections when at least one group has tasks', () => {
    ...
    renderSections(groups, makeCtx());
    expect(document.querySelector('[data-group="today"]').hidden).toBe(false);
    expect(document.querySelector('[data-group="tomorrow"]').hidden).toBe(false);
    expect(document.querySelector('[data-group="thisWeek"]').hidden).toBe(false);
  });

  it('shows empty state message when a group has no tasks', () => {
    const groups = { today: [makeTask()], tomorrow: [], thisWeek: [] };
    renderSections(groups, makeCtx());
    const tomorrowList = document.querySelector('[data-group="tomorrow"] .content__task-list');
    const empty = tomorrowList.querySelector('.task-list-empty');
    expect(empty).not.toBeNull();
    expect(empty.querySelector('.task-list-empty__title').textContent).toContain('tomorrow');
  });
  ...
  it('clears previous content before rendering', () => {
    const groups = { today: [], tomorrow: [], thisWeek: [] };
    renderSections(groups, makeCtx());
    renderSections({ today: [makeTask({ id: 'new' })], tomorrow: [], thisWeek: [] }, makeCtx());
    const todayList = document.querySelector('[data-group="today"] .content__task-list');
    expect(todayList.children.length).toBe(1);
...
describe('renderFilterSummary', () => {
  ...
  it('displays multiple filter chips when several filters are active', () => {
    const ctx = makeCtx({
      hasActiveFilters: true,
      activeFilters: { status: 'active', list: 'work', tag: 'Urgent', search: 'milk' },
    });
    renderFilterSummary(ctx);
...
describe('renderDetailPanel', () => {
  ...
  it('shows subtask count when subtasks exist', () => {
    ...
    expect(subtaskField.querySelector('.detail-panel__value').textContent).toBe('2 subtasks');
  });
  ...
  it('removes taskId when panel is hidden', () => {
    ...
    expect(document.querySelector('.detail-panel').dataset.taskId).toBeUndefined();
  });
