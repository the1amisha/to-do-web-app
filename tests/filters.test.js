...
describe("hasActiveFilters", () => {
  ...
  it("returns true when status is active", () => {
    expect(hasActiveFilters({ ...DEFAULT_FILTERS, status: "active" })).toBe(
      true,
    );
  });

  it("returns true when status is completed", () => {
    expect(hasActiveFilters({ ...DEFAULT_FILTERS, status: "completed" })).toBe(
      true,
    );
  });
...
describe("getFilteredTasks", () => {
  ...
  it("excludes tasks without tags property when filtering by tag", () => {
    ...
    const result = getFilteredTasks(tasksNoTags, {
      ...DEFAULT_FILTERS,
      tag: "Work",
    });
