...
const sampleTasks = [
  { id: 'a', title: 'Buy milk', completed: false, listId: 'personal', dueDate: '', tags: [], subtasks: [], createdAt: 1 },
  { id: 'b', title: 'Read book', completed: true, listId: 'work', dueDate: '2026-07-20', tags: ['Meeting'], subtasks: [], createdAt: 2 },
];
...
describe('loadTasks', () => {
  ...
  it('applies migrate callback to each task', () => {
    const oldFormat = [{ id: 'a', title: 'Test', list: 'Personal', completed: false }];
    ...
    expect(result).toEqual([{ id: 'a', title: 'Test', listId: 'personal', completed: false }]);
  });
...
describe('storage key isolation', () => {
  ...
  it('tasks and lists use separate keys', () => {
    saveTasks(sampleTasks);
    saveLists(sampleLists);
    expect(localStorage.getItem(TASKS_KEY)).not.toBe(localStorage.getItem(LISTS_KEY));
  });
